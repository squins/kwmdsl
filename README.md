...
* The markup is static just like HTML markup. Dynamic interfaces must use the visibility of components, panel replacement, etc.
...
Here is the [_Hello World!_ example of Wicket](https://wicket.apache.org/learn/examples/helloworld.html) in Kotlin using the DSL. There is a simple mapping from the standard Wicket code to the DSL, so it is easy to convert, and the resulting code is easily recognizable as Wicket code.
...
4. In the markup refer to a component using a reference to the component property.
...
The DSL can be integrated into existing pages and components.
...
Now that the page and component indicate they provide their markup in a custom way, and the companion object defines all markup variants, the only thing left to do is to add the hierarchy of the descendent components to the component. This can simply be done by invoking the appropriate function on the markup:
...
* [`addToFragment(...)` with markup in parent](src/examples/kotlin/kwmdsl/examples/dsl_convenience_base_classes/fragment/OwnComponentsMarkupInParentFragment.kt)
