...
fun KClass<*>.linkPath(scope: KClass<*>, resourceName: String) =
    relativizePath(qualifiedName!!.split('.'), scope.qualifiedName!!.split('.').toMutableList().apply {
        set(size - 1, resourceName)
    }.toList(), '/')

inline fun <reified TScope : Any> KClass<*>.linkPath(resourceName: String) =
    linkPath(TScope::class, resourceName)
